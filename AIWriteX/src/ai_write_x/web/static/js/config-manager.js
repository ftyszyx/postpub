class AIWriteXConfigManager { 
    ...
    createFormGroup(label, type, id, value, placeholder, required = false, readonly = false) {      
        ...
        input.addEventListener('blur', async (e) => {        
            ...
            if (e.target.value !== originalValue) {        
                ...
                // ✅ 图片API配置      
                const editableApiMatch = id.match(/api-(\w+)-(base-url|protocol-type)/);
                if (editableApiMatch) {
                    const [, provider, field] = editableApiMatch;
    ...
    createAPIProviderCard(providerKey, providerDisplay, providerData, currentAPIType) {  
        ...
        if (isCustomProvider) {
            const baseUrlGroup = this.createFormGroup(
                'Base URL',
                'text',
                `api-${providerKey}-base-url`,
                providerData.api_base || '',
                '自定义模型接口地址',
                true,
                false
            );
            baseUrlGroup.classList.add('form-group-half');

            const protocolTypeGroup = this.createAPIProtocolTypeGroup(
                providerKey,
                providerData.protocol_type || 'openai_compatible'
            );

            row1.appendChild(baseUrlGroup);
            row1.appendChild(protocolTypeGroup);
        } else {
        ...
        modelSelectLabel.textContent = isCustomProvider ? 'Model ID' : '模型';  
    ...
    createAPIProtocolTypeGroup(providerKey, selectedValue) {
        const group = document.createElement('div');
        group.className = 'form-group form-group-half';

        const label = document.createElement('label');
        label.setAttribute('for', `api-${providerKey}-protocol-type`);
        label.textContent = 'Protocol Type';
        ...
        select.id = `api-${providerKey}-protocol-type`;
        select.className = 'form-control';

        const options = this.getAPIProtocolTypeOptions();
        ...
        const defaultValue = options.some(option => option.value === selectedValue)
            ? selectedValue
            : 'openai_compatible';
        select.value = defaultValue;

        select.addEventListener('change', async (e) => {
            await this.updateAPIProviderField(providerKey, 'protocol-type', e.target.value);
        });
