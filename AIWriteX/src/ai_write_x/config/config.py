...
class Config:
    ...
    @property
    def api_protocol_type(self):
        with self._lock:
            if not self.config:
                raise ValueError("config not loaded")
            return self.config["api"][self.config["api"]["api_type"]].get(
                "protocol_type", "openai_compatible"
            )
    ...
    def validate_config(self):
        """Validate runtime config before executing CrewAI."""
        try:
            ...
            if not api_keys or not any(api_keys):
                self.error_message = f"Missing API key for {api_type}"
                return False

            key_index = api_config.get("key_index", 0)
            if key_index < 0 or key_index >= len(api_keys):
                self.error_message = (
                    f"{api_type} key_index {key_index} is out of range for api_key list size {len(api_keys)}"
                )
                return False
            ...
            if not models:
                self.error_message = f"Missing model for {api_type}"
                return False

            model_index = api_config.get("model_index", 0)
            if model_index < 0 or model_index >= len(models):
                self.error_message = (
                    f"{api_type} model_index {model_index} is out of range for model list size {len(models)}"
                )
                return False
            ...
            if api_type == "Custom":
                ...
                if not str(api_base).strip():
                    self.error_message = "Custom api_base is required"
                    return False

                protocol_type = api_config.get("protocol_type", "openai_compatible")
                if protocol_type not in ALLOWED_API_PROTOCOL_TYPES:
                    self.error_message = (
                        "Unsupported Custom protocol_type. Allowed values: "
                        f"{', '.join(sorted(ALLOWED_API_PROTOCOL_TYPES))}"
                    )
                    return False
            ...
            if self.img_api_type != "picsum":
                ...
                if (
                    ...
                    self.error_message = f"Missing image API key for {self.img_api_type}"
                    return False
                ...
                if (
                    ...
                    self.error_message = f"Missing image model for {self.img_api_type}"
                    return False
            ...
            if self.auto_publish:
                ...
                if not valid_cred:
                    self.error_message = "Auto publish requires a wechat appid and appsecret"
                    return False

            if not self.aiforge_api_key:
                log.print_log("AIForge has no valid llm provider api_key, search will be skipped")

            return True

        except Exception as e:
            self.error_message = f"Config validation failed: {e}"
            return False
