...
class AgentFactory:
    ...
    def _build_global_llm_kwargs(self, config: Config) -> Dict[str, Any]:
        api_provider_config = config.config["api"][config.api_type]
        llm_kwargs: Dict[str, Any] = {
            "model": config.api_model,
            "api_key": config.api_key,
            "max_tokens": api_provider_config.get("max_tokens", 8192),
            "timeout": 120,
        }
    ...
    def _get_llm(self, llm_config: Dict[str, Any] | None = None) -> Optional[LLM]:
        ...
        if not llm_config:
            cache_key = (
                f"{config.api_type}_{config.api_model}_"
                f"{config.api_apibase}_{config.api_protocol_type}"
            )
            if cache_key not in self._llm_cache:
                if config.api_key:
                    self._llm_cache[cache_key] = LLM(
                        **self._build_global_llm_kwargs(config)
                    )
                else:
                    return None
