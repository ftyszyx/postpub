...
- **全方位一致性诊断**：专业级`AI 深度全方位诊断`，像一位白金主编随身指导，精准指出逻辑冲突并给出专业修正方案。
- **情感律动曲线**：可视化`预期情绪起伏`与`AI 写作情绪图表`，精准掌控读者的肾上腺素。
- **极致工作流**：支持`一键极速初始化`（设定/主角/大纲/卷结构）和`去 AI 味处理`，让 AI 创作更有"人味"。
...
> Note: `crewai==0.102.0` depends on `litellm==1.60.2`, which is unavailable on many package indexes. This repo now pins `crewai==1.0.0` so `uv pip install -r requirements.txt` can resolve successfully.
