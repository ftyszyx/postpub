...
fn parse_command_inner(args: &[String], flags: &Flags) -> Result<Value, ParseError> {
    ...
    match cmd {
        ...
        "open" | "goto" | "navigate" => {
            ...
            let mut explicit_wait_until = None;
            let mut explicit_timeout = None;
            let mut i = 1;
            while i < rest.len() {
                match rest[i] {
                    "--wait-until" => {
                        if let Some(wait_until) = rest.get(i + 1) {
                            explicit_wait_until = Some(*wait_until);
                            i += 1;
                        } else {
                            return Err(ParseError::MissingArguments {
                    ...
                    "--timeout" => {
                        if let Some(timeout) = rest.get(i + 1) {
                            match timeout.parse::<u64>() {
                                Ok(ms) => {
                                    explicit_timeout = Some(ms);
                                    i += 1;
                                }
                                Err(_) => {
                                    return Err(ParseError::InvalidValue {
                                        ...
                                        usage:
                                            "open <url> [--wait-until <load|domcontentloaded|networkidle|none>] [--timeout <ms>]",
                                    });
                                }
                            }
                        } else {
                            return Err(ParseError::MissingArguments {
                                context: "open --timeout".to_string(),
            ...
            if let Some(wait_until) = explicit_wait_until {
                nav_cmd["waitUntil"] = json!(wait_until);
            } else if flags.provider.is_some() {
        ...
        // === Wait ===
        "wait" => {
            let explicit_timeout = if let Some(idx) = rest.iter().position(|&s| s == "--timeout") {
                let timeout = rest
                    .get(idx + 1)
                    .ok_or_else(|| ParseError::MissingArguments {
...
mod tests {
    ...
    fn test_navigate_with_wait_until() {
        let cmd = parse_command(
            &args("open example.com --wait-until domcontentloaded"),
            &default_flags(),
        )
        .unwrap();
        assert_eq!(cmd["action"], "navigate");
    ...
    fn test_navigate_with_timeout() {
        let cmd =
            parse_command(&args("open example.com --timeout 3000"), &default_flags()).unwrap();
        assert_eq!(cmd["action"], "navigate");
        assert_eq!(cmd["url"], "https://example.com");
    ...
    #[test]
    fn test_navigate_with_wait_until_and_timeout() {
        let cmd = parse_command(
            &args("open example.com --wait-until domcontentloaded --timeout 3000"),
            &default_flags(),
        )
        .unwrap();
        assert_eq!(cmd["waitUntil"], "domcontentloaded");
    ...
    #[test]
    fn test_navigate_with_provider_respects_explicit_wait_until() {
        let mut flags = default_flags();
    ...
    #[test]
    fn test_navigate_with_provider_defaults_wait_until_none() {
        let mut flags = default_flags();
        flags.provider = Some("browserbase".to_string());
    ...
    #[test]
    fn test_navigate_with_wait_until_requires_value() {
        let result = parse_command(&args("open example.com --wait-until"), &default_flags());

        assert!(result.is_err());
        let err = result.unwrap_err();
        let msg = err.format();
        assert!(msg.contains("open --wait-until"));
    }

    #[test]
    fn test_navigate_with_timeout_requires_value() {
        let result = parse_command(&args("open example.com --timeout"), &default_flags());

        assert!(result.is_err());
        let err = result.unwrap_err();
        let msg = err.format();
        assert!(msg.contains("open --timeout"));
    }

    #[test]
    fn test_navigate_with_timeout_requires_numeric_value() {
        let result = parse_command(&args("open example.com --timeout nope"), &default_flags());

        assert!(result.is_err());
        let err = result.unwrap_err();
        let msg = err.format();
        assert!(msg.contains("Timeout must be a non-negative integer"));
    }
    ...
    #[test]
    fn test_wait_load_with_timeout() {
        let cmd = parse_command(
            &args("wait --load domcontentloaded --timeout 3000"),
            &default_flags(),
        )
        .unwrap();
        assert_eq!(cmd["action"], "waitforloadstate");
        assert_eq!(cmd["state"], "domcontentloaded");
        assert_eq!(cmd["timeout"], 3000);
    }

    #[test]
    fn test_wait_load_with_timeout_requires_value() {
        let result = parse_command(
            &args("wait --load domcontentloaded --timeout"),
            &default_flags(),
        );
        ...
        let err = result.unwrap_err();
        let msg = err.format();
        assert!(msg.contains("wait --timeout"));
    }
