...
#[derive(Embed)]
#[folder = "../../agent-browser/packages/dashboard/out/"]
...
pub(super) fn cors_headers_for_origin(origin: Option<&str>) -> String {
    let allowed_origin = match origin {
        Some(origin) if super::is_allowed_origin(Some(origin)) => origin,
        _ => "http://localhost",
    };
    format!(
...
pub(super) async fn relay_command_to_daemon(
    ...
    let mut json_str = serde_json::to_string(&command).map_err(|error| error.to_string())?;
    json_str.push('\n');

    #[cfg(unix)]
    let stream = {
        let socket_path = get_socket_dir().join(format!("{session_name}.sock"));
...
pub(super) fn serve_embedded_file(url_path: &str) -> (&'static str, &'static str, Vec<u8>) {
    ...
    match file {
        Some(content) => {
            ...
            ("200 OK", content_type, content.data.to_vec())
        }
        None => (
            "404 Not Found",
