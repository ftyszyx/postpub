...
impl BrowserManager {
    ...
    pub async fn navigate(
        ...
        if nav_result.loader_id.is_some() && wait_until != WaitUntil::None {
            self.wait_for_lifecycle(wait_until, timeout_ms, &session_id, &mut lifecycle_rx)
                .await?;
        }
    ...
    async fn wait_for_lifecycle(
        ...
        let timeout =
            tokio::time::Duration::from_millis(timeout_ms.unwrap_or(self.default_timeout_ms));
    ...
    pub async fn wait_for_lifecycle_external(
        ...
        self.wait_for_lifecycle(wait_until, None, session_id, &mut rx)
            .await
    }
