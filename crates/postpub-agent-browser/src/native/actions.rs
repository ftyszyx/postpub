...
async fn handle_navigate(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
    ...
    let timeout_ms = cmd.get("timeout").and_then(|v| v.as_u64());
    ...
    mgr.navigate(url, wait_until, timeout_ms).await
}
...
async fn handle_diff_url(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
    ...
    mgr.navigate(url1, wait_until, None).await?;
    let session_id = mgr.active_session_id()?.to_string();
    ...
    mgr.navigate(url2, wait_until, None).await?;
    state.ref_map.clear();
...
async fn handle_auth_login(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
    ...
    let mgr = state.browser.as_mut().ok_or("Browser not launched")?;
    mgr.navigate(&url, AUTH_LOGIN_WAIT_UNTIL, None).await?;
